#!/usr/bin/env python3
"""V18: Squeeze more from DW stacking. V17c got 0.3250.
Ideas: Add TabPFN, GP to stack. Try non-linear stackers. Feature-augmented stacking."""
import pandas as pd, numpy as np, warnings, time, os, pickle
...
def stack_cv(oof_mat, y, model_fn, scale=False, n_rep=5):
    """Stack with CV."""
    bins = make_bins(y)
    ns = len(y); sp = np.zeros(ns); sc = np.zeros(ns)
    for tr,te in RepeatedStratifiedKFold(n_splits=5,n_repeats=n_rep,random_state=42).split(oof_mat,bins):
        Xtr, Xte = oof_mat[tr], oof_mat[te]
        if scale:
            s = StandardScaler(); Xtr = s.fit_transform(Xtr); Xte = s.transform(Xte)
        m = model_fn(); m.fit(Xtr, y[tr]); sp[te] += m.predict(Xte); sc[te] += 1
    sp /= sc
    return 1 - np.sum((y-sp)**2)/np.sum((y-y.mean())**2), sp
...
def run_target(target_col, target_name, target_r2):
    ...
    # === PHASE 1: Generate diverse base model predictions (fast models only) ===
    fast_models=[
    ...
    for fs_name, X_fs in fsets.items():
        ...
        # Print top 5 for this feature set
        fs_results = {k:v for k,v in all_results.items() if k.startswith(fs_name)}
        top5 = sorted(fs_results.keys(), key=lambda k: fs_results[k]['r2'], reverse=True)[:5]
        for nm in top5:
            print(f"  {nm:50s}: R2={fs_results[nm]['r2']:.4f}", flush=True)
    
    # === PHASE 2: Feature-augmented stacking ===
    # Key idea: concatenate raw features WITH model predictions for stacking
    good = {k:v for k,v in all_results.items() if v['r2'] > 0}
    names = sorted(good.keys(), key=lambda k: good[k]['r2'], reverse=True)[:25]
    preds_mat = np.column_stack([good[nm]['preds'] for nm in names])
    
    print(f"\n--- Stacking {len(names)} models ---", flush=True)
    print(f"  Top 5:", flush=True)
    for nm in names[:5]:
        print(f"    {nm:50s}: R2={good[nm]['r2']:.4f}", flush=True)
    
    ss_tot = np.sum((y-y.mean())**2)
    best_r2 = -999; best_preds = None
    all_stacks = {}
    
    # --- Pure prediction stacking ---
    for alpha in [0.001,0.01,0.1,0.5,1,5,10,50,100,500,1000,5000]:
        r2, sp = stack_cv(preds_mat, y, lambda a=alpha: Ridge(alpha=a), n_rep=5)
    ...
    for alpha in [0.01,0.1,1]:
        for l1 in [0.1,0.3,0.5,0.7,0.9]:
            r2, sp = stack_cv(preds_mat, y, lambda a=alpha,l=l1: ElasticNet(alpha=a,l1_ratio=l,max_iter=5000,positive=True), n_rep=5)
            if r2 > best_r2: best_r2=r2; best_preds=sp.copy()
            if r2 > 0.20: all_stacks[f'en_{alpha}_{l1}']=sp.copy()
    print(f"  +EN: R2={best_r2:.4f}", flush=True)
    
    for alpha in [0.001,0.01,0.1]:
        r2, sp = stack_cv(preds_mat, y, lambda a=alpha: Lasso(alpha=a,max_iter=5000,positive=True), n_rep=5)
        if r2 > best_r2: best_r2=r2; best_preds=sp.copy()
        if r2 > 0.20: all_stacks[f'lasso_{alpha}']=sp.copy()
    print(f"  +Lasso: R2={best_r2:.4f}", flush=True)
    
    for k in [3,5,7,10,15,20,30]:
        r2, sp = stack_cv(preds_mat, y, lambda kk=k: KNeighborsRegressor(n_neighbors=kk,weights='distance'), scale=True, n_rep=3)
        if r2 > best_r2: best_r2=r2; best_preds=sp.copy()
    ...
    for C in [0.1,1,10,100]:
        for eps in [0.05,0.1]:
            r2, sp = stack_cv(preds_mat, y, lambda c=C,e=eps: SVR(kernel='rbf',C=c,epsilon=e), scale=True, n_rep=3)
            if r2 > best_r2: best_r2=r2; best_preds=sp.copy()
            if r2 > 0.20: all_stacks[f'svr_{C}_{eps}']=sp.copy()
    print(f"  +SVR: R2={best_r2:.4f}", flush=True)
    
    for d in [2,3]:
        r2, sp = stack_cv(preds_mat, y, lambda dd=d: xgb.XGBRegressor(n_estimators=100,max_depth=dd,learning_rate=0.05,reg_alpha=5,reg_lambda=10,random_state=42), n_rep=3)
        if r2 > best_r2: best_r2=r2; best_preds=sp.copy()
        if r2 > 0.20: all_stacks[f'xgb_{d}']=sp.copy()
    print(f"  +XGB: R2={best_r2:.4f}", flush=True)
    
    r2, sp = stack_cv(preds_mat, y, BayesianRidge, n_rep=5)
    if r2 > best_r2: best_r2=r2; best_preds=sp.copy()
    if r2 > 0.20: all_stacks['bayesian']=sp.copy()
    print(f"  +Bayesian: R2={best_r2:.4f}", flush=True)
    
    # --- Feature-augmented stacking: predictions + raw features ---
    print(f"\n  Feature-augmented stacking...", flush=True)
    X_aug = np.column_stack([preds_mat, StandardScaler().fit_transform(X_raw)])
    for alpha in [0.01,0.1,1,10,100,500]:
        r2, sp = stack_cv(X_aug, y, lambda a=alpha: Ridge(alpha=a), n_rep=5)
        if r2 > best_r2: best_r2=r2; best_preds=sp.copy()
        if r2 > 0.28: all_stacks[f'aug_ridge_{alpha}']=sp.copy()
    print(f"  Aug Ridge: R2={best_r2:.4f}", flush=True)
    
    for C in [1,10,100]:
        r2, sp = stack_cv(X_aug, y, lambda c=C: SVR(kernel='rbf',C=c,epsilon=0.1), scale=True, n_rep=3)
        if r2 > best_r2: best_r2=r2; best_preds=sp.copy()
        if r2 > 0.28: all_stacks[f'aug_svr_{C}']=sp.copy()
    print(f"  Aug SVR: R2={best_r2:.4f}", flush=True)
    
    for d in [2,3]:
        r2, sp = stack_cv(X_aug, y, lambda dd=d: xgb.XGBRegressor(n_estimators=100,max_depth=dd,learning_rate=0.05,reg_alpha=5,reg_lambda=10,random_state=42), n_rep=3)
        if r2 > best_r2: best_r2=r2; best_preds=sp.copy()
        if r2 > 0.28: all_stacks[f'aug_xgb_{d}']=sp.copy()
    print(f"  Aug XGB: R2={best_r2:.4f}", flush=True)
    
    # --- Mega blend ---
    rng = np.random.RandomState(42)
    all_preds = np.array([good[nm]['preds'] for nm in names])
    blend_best = -999
    for _ in range(500000):
        w=rng.dirichlet(np.ones(len(names))*0.3); bl=w@all_preds; r2=1-np.sum((y-bl)**2)/ss_tot
        if r2>blend_best: blend_best=r2
    for _ in range(200000):
        w=rng.dirichlet(np.ones(len(names))*0.1); bl=w@all_preds; r2=1-np.sum((y-bl)**2)/ss_tot
        if r2>blend_best: blend_best=r2
    for i in range(len(names)):
        for j in range(i+1,len(names)):
            for a in np.linspace(0,1,201):
                bl=a*all_preds[i]+(1-a)*all_preds[j]; r2=1-np.sum((y-bl)**2)/ss_tot
                if r2>blend_best: blend_best=r2
    for i in range(min(8,len(names))):
        for j in range(i+1,min(8,len(names))):
            for k in range(j+1,min(8,len(names))):
                for a in np.linspace(0.05,0.9,20):
                    for b in np.linspace(0.05,0.9-a,15):
                        c=1-a-b
                        if c>0:
                            bl=a*all_preds[i]+b*all_preds[j]+c*all_preds[k]
    ...
    if len(all_stacks) >= 3:
        ...
        smat = np.column_stack([all_stacks[k] for k in all_stacks])
        for alpha in [0.01,0.1,1,10,100]:
            r2, sp = stack_cv(smat, y, lambda a=alpha: Ridge(alpha=a), n_rep=5)
            if r2 > best_r2: best_r2=r2
        # Feature-augmented layer 2
        smat_aug = np.column_stack([smat, StandardScaler().fit_transform(X_raw)])
        for alpha in [0.1,1,10,100]:
            r2, sp = stack_cv(smat_aug, y, lambda a=alpha: Ridge(alpha=a), n_rep=5)
            if r2 > best_r2: best_r2=r2
        print(f"  Layer 2: R2={best_r2:.4f}", flush=True)
    
    # --- Multi-seed CV stability check ---
    print(f"\n  Stability check with different seeds...", flush=True)
    for seed in [0, 1, 2, 3]:
        bins = make_bins(y)
        sp = np.zeros(len(y)); sc = np.zeros(len(y))
        for tr,te in RepeatedStratifiedKFold(n_splits=5,n_repeats=5,random_state=seed).split(preds_mat,bins):
            m = Ridge(alpha=10); m.fit(preds_mat[tr], y[tr])
            sp[te] += m.predict(preds_mat[te]); sc[te] += 1
        sp /= sc
        r2 = 1-np.sum((y-sp)**2)/ss_tot
        print(f"  seed={seed}: R2={r2:.4f}", flush=True)
    
    print(f"\n  >>> {target_name} FINAL: R2={best_r2:.4f} (target {target_r2}, gap={target_r2-best_r2:.3f})", flush=True)
    return best_r2

print("="*60)
print("V18: SQUEEZE MORE FROM DW STACKING")
print("="*60)
t0=time.time()
...
print(f"\n{'='*60}")
print(f"V18 FINAL ({(time.time()-t0)/60:.1f} min)")
print(f"{'='*60}")
...
print(f"  V17c: HOMA=0.3250, hba1c=0.1677")
